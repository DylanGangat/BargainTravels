...
const updateMockupColor = e => {
  if (e.target.classList.contains("colour-block")) {
    ...
    const imageId = e.target.dataset.color; // To select the image that i want to display

    const image = document.querySelector(`${imageId}`); // To reset the images so they are all hidden
